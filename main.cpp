...
int main(int argc, char **argv) {
    ...
    auto yolov8_pose = new YoloV8Pose(enginePath);
    yolov8_pose->makePipe(false);
    ...
    if (isVideo) {
        ...
        while (cap.read(image)) {
            objs.clear();
            yolov8_pose->copyFromMat(image, size);
            auto start = std::chrono::system_clock::now();
            yolov8_pose->infer();
            auto end = std::chrono::system_clock::now();
            yolov8_pose->postprocess(objs, scoreThreshold, IOThreshold, topk);
            yolov8_pose->drawObjects(image, res, objs, SKELETON, KPS_COLORS, LIMB_COLORS);
            auto tc = (double)
